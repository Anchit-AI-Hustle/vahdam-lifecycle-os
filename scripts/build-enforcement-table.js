...
const CLAIMS = [
  ...
  { group: 'One source of truth',
    claim: 'No page keeps its own store map either; the guard reads the pages, not just the modules.',
    spec: 'market-urls.spec.js', test: 'no page keeps its own market' },
  { group: 'One source of truth',
    claim: 'Every surface that hands a human a prompt reads it from the API, not a local copy.',
    spec: 'asset-vs-element-prompts.spec.js', test: 'holds no copy of one' },

  { group: 'Assets are finished, not briefed',
    claim: 'Every asset prompt returns the complete deliverable; a brief is a failed response.',
    spec: 'asset-vs-element-prompts.spec.js', test: 'the deliverable is the whole asset' },
  { group: 'Assets are finished, not briefed',
    claim: 'Copying an ad prompt returns the creative itself, not a description of one.',
    spec: 'asset-vs-element-prompts.spec.js', test: 'returns the creative, not a brief' },

  { group: 'The daily loop actually runs',
    claim: "The Smart Brain's rolling calendar has its own daily schedule, not a step at the end of another job.",
    spec: 'smart-brain-cron.spec.js', test: 'a scheduled cron reaches the Smart Brain plan sync' },
  { group: 'The daily loop actually runs',
    claim: 'A daily run that runs out of time reports what it skipped instead of vanishing.',
    spec: 'smart-brain-cron.spec.js', test: 'skips every step and says so' },
  { group: 'The daily loop actually runs',
    claim: 'A sync that cannot finish commits what it did and defers the rest to the next run.',
    spec: 'smart-brain-cron.spec.js', test: 'defers rows instead of being killed' },
  { group: 'The daily loop actually runs',
    claim: 'Every send is planned from its own market catalog, never another region\'s.',
    spec: 'smart-brain-market-products.spec.js', test: 'planned from its own market catalog' },
