...
| 32 | **LLM Waterfall** (`llm.js`) | 82 | 95 | eleven-rung cascade (Anthropic → OpenAI → Gemini → Grok → Groq → Cerebras → GitHub Models → Cloudflare → OpenRouter → Ollama → Sakana; last five conditional on env); gap: model IDs may drift | Add `/api/health?probe=1` provider health check; add model-ID freshness audit in CI; add cost-per-call tracking |
