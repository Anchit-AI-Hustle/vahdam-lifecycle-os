...
- **USA July calendar + mailers** (`npm run build:july`): `scripts/build-july-mailers.js` runs the *exact* automated-calendar mailer logic — the same `helpers.renderTextVariant` renderer (`calendar-trigger.js`) and the same `sanitizeBrand`/`assertNoBanned` gates (`scenario-model.js`) that `lifecycle-mailer-build.js` uses — producing all four variants per slot (`text_a` pure, `text_b` editorial, `visual_a`/`visual_b` visual). 12 cohort sends × 4 = 48 files in `mailers/usa-july/`; hero images come ONLY from verified `brand_assets` rows (image-free otherwise, never a fake URL). `scripts/build-july-studio.js` assembles `vahdam-usa-july-calendar-mailer-studio.html` (served at `/july-studio` · `/usa-july`): row-per-send, scenario tabs (C = executed model, 2-3 emails/user/week), variant preview = the exact embedded downloadable file, plus the data-grounded reasoning per row. Manifest: `data/calendar/usa-july-2026.json`. Event hooks wired into reasoning: WC Final Jul 19 @ MetLife, National Ice Cream Day Jul 19, Parents' Day Jul 26, Int'l Day of Friendship Jul 30, National Wellness Month (Aug) ramp.
