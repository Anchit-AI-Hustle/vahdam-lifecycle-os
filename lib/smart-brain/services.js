  ...
  generate({ analysis, competitorBenchmarks, startDate = todayIso(), days = this.config.calendarDays, feedback = [] }) {
    ...
    for (let d = 0; d < days; d++) {
      ...
      for (const market of this.config.markets) {
        ...
        // (Matches the product contract in the console: "the full funnel —
        // mailer, Meta/Google/TikTok ads, and a hosted landing page".) Emergency
        // standby still narrows to owned channels via effectiveEntry().
