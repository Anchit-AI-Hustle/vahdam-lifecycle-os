  ...
  generate({ analysis, competitorBenchmarks, startDate = todayIso(), days = this.config.calendarDays, feedback = [] }) {
    ...
    for (let d = 0; d < days; d++) {
      ...
      for (const market of this.config.markets) {
        ...
        // (Matches the product contract in the console.) Emergency standby still
        // narrows to owned channels via effectiveEntry(). Used consistently by
        // the MVT plan, competitor context, analysis and the pushed entry below.
        ...
        const reach = {
          ...
          min_recipients: fq.minRecipientsPerSend || 20000,
          target_recipients: fq.targetRecipientsPerSend || 30000,
          planned_recipients: Math.max(fq.minRecipientsPerSend || 20000, Math.min(cohortSize || (fq.targetRecipientsPerSend || 30000), fq.targetRecipientsPerSend || 30000)),
          meets_floor_from_core: cohortSize >= (fq.minRecipientsPerSend || 20000),
          per_user_per_week: { min: fq.minPerUserPerWeek || 2, max: fq.maxPerUserPerWeek || 3 },
          widen_note: cohortSize >= (fq.minRecipientsPerSend || 20000)
            ? 'Core cohort alone meets the recipient floor.'
            : `Core cohort (${cohortSize}) is below the ${fq.minRecipientsPerSend || 20000} floor; widen to adjacent/broad audiences at ESP to reach the minimum.`,
        };
        ...
        // Explainable confidence — same factors/weights as the legacy
        // confidenceFor(), but surfaced as a factor-by-factor breakdown so the
        // plan can show WHY a slot scores what it does. Score stays identical.
        const conf = explainConfidence([
          ...
          { when: ((cohort.count || 0) > 1000), label: 'Large addressable cohort', delta: 0.08, detail: `${cohort.count} profiles gives statistically stable reach.` },
        ...
        // Complete, data-grounded analysis attached to every entry (the "why").
