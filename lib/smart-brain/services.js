...
// Default daily attributed-email revenue target (spec §24a). Drives per-slot
// revenue-feasibility so sub-threshold cohorts are flagged, not assumed viable.
const DAILY_REVENUE_TARGET_USD = 1500;
    ...
    for (let d = 0; d < days; d++) {
      ...
      for (const market of this.config.markets) {
        ...
        for (let k = 0; k < perDay; k++) {
          ...
          const perSendTarget = round((DAILY_REVENUE_TARGET_USD / Math.max(1, this.config.cohortsPerDay || 3)), 0);
            ...
            status: !hasRealBase
              ...
              : (projectedRevenue >= perSendTarget
                ...
                : ((cohortSize >= minR * 0.25 || projectedRevenue >= perSendTarget * 0.5)
                  ...
                  : 'NOT FEASIBLE')),
            note: hasRealBase
              ? `~${cohortSize} recipients × $${rpr}/recipient ≈ $${projectedRevenue} vs ~$${perSendTarget}/send target.`
              : 'No real eligible-segment size yet (wire the ESP feed, B3).',
          };
