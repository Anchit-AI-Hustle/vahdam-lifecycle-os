...
function buildAdAssets(entry, campaignId) {
  ...
  const prodLed = safeCopy(`${hero}, chosen for ${cohort}. The VAHDAM India edit.`);
  const benefitLed = safeCopy(`The daily ritual ${cohort} keep coming back to. Meet ${hero}.`);
  ...
  const videoAd = (platform, placement) => ({
    ...
    hook: safeCopy(`${cohort}: your calmer daily ritual starts here`),
    storyboard: [
  ...
  const out = [];
  // Every enabled channel ships BOTH a Static and a Video creative.
  if (entry.channels.includes('meta'))   out.push(staticAd('meta', '1:1', 'Feed 1:1 / 4:5'), videoAd('meta', 'Reels / Stories'));
  if (entry.channels.includes('google')) out.push(staticAd('google', '1.91:1', 'Responsive Display / Demand Gen'), videoAd('google', 'YouTube Shorts / Demand Gen'));
  if (entry.channels.includes('tiktok')) out.push(videoAd('tiktok', 'TikTok In-Feed'), staticAd('tiktok', '9:16', 'TikTok image card'));
  return out;
}
