...
const METRICS = [
  ...
  { key: 'rm_ctr', label: 'CTR (retail)', category: 'retail', unit: 'pct', tier: 'derived', inputs: ['clicks', 'impressions'], formula: 'clicks / impressions × 100 (marketplace report)', compute: (r) => pct(r.clicks, r.impressions), def: 'Click-through rate on the marketplace placement.' },
  ...
  { key: 'rm_cpm', label: 'CPM (retail)', category: 'retail', unit: 'usd', tier: 'derived', inputs: ['cost', 'impressions'], formula: 'cost / impressions × 1000', compute: (r) => { const d = div(r.cost, r.impressions); return d == null ? null : d * 1000; }, def: 'Cost per thousand marketplace impressions.' },
  { key: 'acos', label: 'ACOS (advertising cost of sales)', category: 'retail', unit: 'pct', tier: 'derived', inputs: ['cost', 'attributed_sales'], formula: 'ad cost / attributed sales × 100', compute: (r) => pct(r.cost, r.attributed_sales), def: 'The defining retail-media efficiency metric: share of attributed revenue consumed by ad spend. LOWER is better - the inverse of ROAS.' },
  ...
  { key: 'tacos', label: 'TACOS (total advertising cost of sales)', category: 'retail', unit: 'pct', tier: 'derived', inputs: ['cost', 'total_revenue'], formula: 'ad cost / TOTAL revenue × 100', compute: (r) => pct(r.cost, r.total_revenue), def: 'Ad cost against TOTAL (organic + paid) marketplace revenue - the health metric ACOS cannot show. Needs total revenue joined in.' },
  { key: 'rm_cvr', label: 'Conversion rate (retail)', category: 'retail', unit: 'pct', tier: 'derived', inputs: ['attributed_orders', 'clicks'], formula: 'attributed orders / clicks × 100', compute: (r) => pct(r.attributed_orders, r.clicks), def: 'Orders per marketplace click - the listing closing rate.' },
