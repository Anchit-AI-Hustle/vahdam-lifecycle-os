...
async function view(name,params){switch(String(name||'status').toLowerCase()){case'ads':return ads(params);case'mailer':return mailer(params);case'landing':case'pagedeck':return landing(params);case'actions':return actions();case'summary':case'live-summary':return require('./live-summary-core.js').summary(params);case'revenue':return require('./revenue-analysis-core.js').revenue(params);case'agents':return require('./platform-agents-core.js').runAll(params);case'agent':return require('./platform-agents-core.js').runAgent((params&&params.platform)||'shopify',params);case'alerts':return{ok:true,settings:await loadSettings(),status:await status()};default:return status();}}
